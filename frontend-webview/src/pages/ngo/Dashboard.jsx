                    ...
                    <div key={donation._id} className="border border-gray-200 dark:border-gray-700 rounded-xl p-4 bg-white dark:bg-gray-800 hover:shadow-md transition-shadow">
                      <div className="flex gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {/* Previous release reasons */}
                          {donation.release_history?.length > 0 && (
                            <div className="mt-2 space-y-1">
                              {donation.release_history.map((r, i) => (
                                <div key={i} className="flex items-start gap-2 text-xs bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg px-2.5 py-1.5">
                                  <svg className="w-3.5 h-3.5 text-orange-500 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01M12 3a9 9 0 100 18A9 9 0 0012 3z" /></svg>
                                  <span className="text-orange-700 dark:text-orange-300">
                                    <strong>{t('dashboard.ngo.releaseHistoryLabel')}:</strong> {r.reason}
                                    {r.released_at && <span className="text-orange-500 ml-1">({new Date(r.released_at).toLocaleString('en-IN', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })})</span>}
                                  </span>
                                </div>
                              ))}
                            </div>
