              ...
              {selectedDonation.status === 'collected' && !ngoRating.submitted && (
                <div className="mb-5 bg-amber-50 dark:bg-amber-900/20 border-2 border-amber-300 dark:border-amber-700 rounded-xl p-4">
                  ...
                  <div className="flex gap-1.5 mb-2">
                    {[1,2,3,4,5].map(s => {
                      const active    = s <= (ngoRating.hover || ngoRating.stars);
                      const isHover   = s === ngoRating.hover;
                      const isCascade = ngoRating.hover > 0 && s < ngoRating.hover;
                      const isLocked  = s <= ngoRating.stars && ngoRating.hover === 0;
                      ...
                      return (
                        <button key={s} onMouseEnter={() => setNgoRating(r => ({ ...r, hover: s }))}
                          ...
                          <svg width="36" height="36" viewBox="0 0 24 24" style={{
                            ...
                            filter: isLocked ? 'drop-shadow(0 2px 5px rgba(245,158,11,0.5))' : 'none',
                          }}>
                  ...
                  <div style={{ minHeight: '22px' }} className="mb-2">
                    {(ngoRating.hover || ngoRating.stars) > 0 && (
                      <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-amber-200 dark:bg-amber-800/40 text-amber-800 dark:text-amber-200"
                        style={{ animation: 'fadeIn 0.15s ease' }}>
                        {['','Poor','Below avg','Good','Very good','Excellent'][ngoRating.hover || ngoRating.stars]}
                      </span>
                    )}
                  </div>
                  {ngoRating.stars > 0 && (
